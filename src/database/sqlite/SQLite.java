...
public class SQLite {
	...
	public SQLite(EKitaManager app){
		this.app = app;
	...
	public void connect(String directory, String database) {		
		...
		try {
			...
			this.conn = DriverManager.getConnection("jdbc:sqlite:" + EKitaManager.getInstance().getDataFolder() + File.separator + directory + File.separator + database);
			//EKitaManager.getInstance().log.LogDebug("Successfully connected to Database.");
		} catch (Exception e) {
			EKitaManager.getInstance().log.LogError("", e);
		}
	...
	public void disconnect() {
		...
		try {
			if(stmt != null)
				this.stmt.close();
			this.conn.close();
			//EKitaManager.getInstance().log.LogDebug("Successfully disconnected from Database.");
		} catch (Exception e) {
			EKitaManager.getInstance().log.LogError("", e);
		}
